...
export const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-[94%] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            {/* Logo link */}
            <Link href="/" className="relative flex-shrink-0" onClick={closeMenu}>
              <div className="relative py-1">
                {!scrolled && (
                  <div className="absolute inset-x-[-30px] inset-y-[-15px] bg-black/80 rounded-full blur-2xl pointer-events-none" />
                )}
                <Logo className="relative z-10" variant="main" size={scrolled ? 240 : 220} />
              </div>
            </Link>
            ...
            <div className="lg:hidden flex items-center">
              <button
                ...
                className={`p-2 transition-colors focus:outline-none ${scrolled || isOpen
                    ? "text-charcoal hover:text-gold"
                    : "text-white/90 hover:text-gold"
                  }`}
      ...
      {/* Mobile Drawer Overlay - Placed outside of header tag to prevent parent transition/transform stacking context issues */}
      <div
        className={`fixed inset-0 top-[60px] bg-white z-40 transition-transform duration-300 lg:hidden border-t border-orange/10 ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
